        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <select name="route_id" class="form-select @error('route_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($routes as $route)
                                        <option value="{{ $route->id }}" {{ old('route_id', $trip->route_id) == $route->id ? 'selected' : '' }}>{{ $route->code }} - {{ $route->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="trip_code" value="{{ old('trip_code', $trip->trip_code) }}" class="form-control @error('trip_code') is-invalid @enderror" required>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="driver_id" class="form-select @error('driver_id') is-invalid @enderror">
                                    ...
                                    @foreach($drivers as $driver)
                                        <option value="{{ $driver->id }}" {{ old('driver_id', $trip->driver_id) == $driver->id ? 'selected' : '' }}>{{ $driver->driver_code }} - {{ $driver->user?->name ?? '-' }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="vehicle_id" class="form-select @error('vehicle_id') is-invalid @enderror">
                                    ...
                                    @foreach($vehicles as $vehicle)
                                        <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $trip->vehicle_id) == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->vehicle_code }} - {{ $vehicle->plate_number }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="schedule_id" class="form-select @error('schedule_id') is-invalid @enderror">
                                    ...
                                    @foreach($schedules as $schedule)
                                        <option value="{{ $schedule->id }}" {{ old('schedule_id', $trip->schedule_id) == $schedule->id ? 'selected' : '' }}>{{ $schedule->schedule_code }} - {{ $schedule->day_of_week }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="datetime-local" name="started_at" value="{{ old('started_at', $trip->started_at?->format('Y-m-d\TH:i')) }}" class="form-control @error('started_at') is-invalid @enderror">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Selesai</label>
                                <input type="datetime-local" name="ended_at" value="{{ old('ended_at', $trip->ended_at?->format('Y-m-d\TH:i')) }}" class="form-control @error('ended_at') is-invalid @enderror">
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <input type="number" min="0" name="total_passengers" value="{{ old('total_passengers', $trip->total_passengers) }}" class="form-control @error('total_passengers') is-invalid @enderror">
                            </div>
                        </div>

                        <div class="mb-4">
                            <label class="form-label">Catatan</label>
                            <textarea name="notes" rows="4" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $trip->notes) }}</textarea>
            ...
            <div class="col-xl-4">
                <div class="card card-lg">
                    <div class="card-body">
                        <button type="submit" class="btn btn-primary w-100 mb-2"><i class="ti ti-check me-1"></i> Update Trip</button>

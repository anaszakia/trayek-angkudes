            ...
            <form action="{{ route('assignments.store') }}" method="POST">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <select name="driver_id" class="form-select @error('driver_id') is-invalid @enderror" required>
                            ...
                            @foreach($drivers as $driver)
                                <option value="{{ $driver->id }}" {{ old('driver_id') == $driver->id ? 'selected' : '' }}>{{ $driver->user?->name ?? $driver->driver_code }} - {{ $driver->driver_code }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="vehicle_id" class="form-select @error('vehicle_id') is-invalid @enderror" required>
                            ...
                            @foreach($vehicles as $vehicle)
                                <option value="{{ $vehicle->id }}" {{ old('vehicle_id') == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->plate_number }} - {{ $vehicle->vehicle_code }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="datetime-local" name="started_at" value="{{ old('started_at') }}" class="form-control @error('started_at') is-invalid @enderror" required>
                    ...
                    <div class="col-12 d-flex gap-2">
                        <button type="submit" class="btn btn-primary"><i class="ti ti-check me-1"></i> Simpan Assignment</button>
